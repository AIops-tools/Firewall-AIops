"""Encrypted secret storage for firewall-aiops.

Secrets (firewall API keys) are NEVER written to disk in plaintext. They
live in ``~/.firewall-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``FIREWALL_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "firewall-aiops"
CONFIG_DIR = Path.home() / ".firewall-aiops"
MASTER_PASSWORD_ENV = "FIREWALL_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``FIREWALL_<TARGET>_SECRET``; ``prefix='FIREWALL_'`` and
