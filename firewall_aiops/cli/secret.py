"""``firewall-aiops secret`` — manage the encrypted credential store.

Secrets (Monitoring API keys) are stored in ``~/.firewall-aiops/secrets.enc``
...
from firewall_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
...
def secret_list() -> None:
    ...
    if not names:
        console.print(
            "[yellow]No secrets stored yet. Add one: firewall-aiops secret set <name>[/]"
        )
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update FIREWALL_AIOPS_MASTER_PASSWORD.[/]")
